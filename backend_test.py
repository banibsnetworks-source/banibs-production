...
class BanibsAPITester:
    ...
    def test_book_vault_update_work(self) -> bool:
        ...
        if not self.admin_token or not hasattr(self, 'test_work_id'):
    ...
    def test_book_vault_entry_detail(self) -> bool:
        ...
        if not self.admin_token or not hasattr(self, 'test_entry_id'):
    ...
    def test_book_vault_versions_list(self) -> bool:
        ...
        if not self.admin_token or not hasattr(self, 'test_new_entry_id'):
            self.log("❌ No admin token or test entry available", "ERROR")
            return False
    ...
    def test_book_vault_role_checking(self) -> bool:
        ...
        if not self.contributor_token:
            self.log("❌ No contributor token available for role test", "ERROR")
            return False
        
        # Test with contributor token (should be denied)
        headers = {"Authorization": f"Bearer {self.contributor_token}"}
        response = self.make_request("GET", "/book-vault/works", headers=headers)
        
        if response.status_code == 403:
            self.log("✅ Role checking working - Contributor properly denied")
            return True
        elif response.status_code == 401:
            self.log("✅ Role checking working - Authentication required")
            return True
        else:
            self.log(f"❌ Role checking failed - Expected 403/401, got {response.status_code}", "ERROR")
            return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if test_name == "adcs":
            ...
            sys.exit(0 if success else 1)
        elif test_name == "bookvault" or test_name == "book-vault":
            # Run Book Vault API tests
