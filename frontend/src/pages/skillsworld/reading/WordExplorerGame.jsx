 ...
 * 
 * CORE MECHANIC:
 * - Show a target word with a hint
 * - Select the correct meaning from options
 * - No timers, no penalties, no failure states
 * - Gentle feedback only ("Try another")
 * 
...
const LEVELS = wordExplorerData.levels;
...
const WordExplorerGame = () => {
  ...
  const [completedLevels, setCompletedLevels] = useState(() => {
    try {
      const saved = localStorage.getItem('banibs_wordexplorer_progress');
      return saved ? JSON.parse(saved) : [];
    } catch {
      return [];
    }
  });
  const [wordsCompletedInLevel, setWordsCompletedInLevel] = useState([]);
  const [isTransitioning, setIsTransitioning] = useState(false);

  const currentLevel = LEVELS[currentLevelIndex];
  const currentWord = currentLevel?.words[currentWordIndex];
  const totalWordsInLevel = currentLevel?.words.length || 0;
  const isLevelComplete = wordsCompletedInLevel.length >= totalWordsInLevel;
  ...
  useEffect(() => {
    if (isLevelComplete && !completedLevels.includes(currentLevel.id)) {
      ...
      try {
        localStorage.setItem('banibs_wordexplorer_progress', JSON.stringify(newCompleted));
      } catch {
        // Silent fail
      }
    }
  ...
  const handleSelectOption = useCallback((option) => {
    ...
    if (correct && !wordsCompletedInLevel.includes(currentWordIndex)) {
      setWordsCompletedInLevel(prev => [...prev, currentWordIndex]);
    }
  }, [currentWord, currentWordIndex, isCorrect, isTransitioning, wordsCompletedInLevel]);

  // Move to next word
  const handleNextWord = useCallback(() => {
    if (currentWordIndex < totalWordsInLevel - 1) {
      setCurrentWordIndex(prev => prev + 1);
      setSelectedOption(null);
      setIsCorrect(null);
      setShowHint(false);
    }
  }, [currentWordIndex, totalWordsInLevel]);
  ...
  // Shuffle options for display (but keep them consistent per word)
  const getShuffledOptions = useCallback(() => {
    if (!currentWord) return [];
    // Use word as seed for consistent shuffle
    const seed = currentWord.word.length + currentWordIndex;
      ...
      >
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            >
              {wordExplorerData.age_guide} • Level {currentLevel.id}
            </p>
          </div>
      ...
      >
        ...
        <div className="w-full max-w-xs mb-6">
          <div className="flex justify-between text-xs mb-2" style={{ color: isDark ? 'rgb(107, 114, 128)' : 'rgb(156, 163, 175)' }}>
            <span>Word {currentWordIndex + 1} of {totalWordsInLevel}</span>
            <span>{wordsCompletedInLevel.length} completed</span>
          </div>
          <div 
          ...
          >
            <div 
              ...
              style={{ 
                width: `${(wordsCompletedInLevel.length / totalWordsInLevel) * 100}%`,
                backgroundColor: isLevelComplete ? 'rgb(34, 197, 94)' : 'rgb(16, 185, 129)'
              }}
            />
          </div>
        </div>
        ...
        {isLevelComplete ? (
          ...
          >
            ...
            <p 
              ...
              style={{ color: isDark ? 'rgb(156, 163, 175)' : 'rgb(107, 114, 128)' }}
            >
              You explored all the words in {currentLevel.name}
            </p>
          ...
          <>
            {/* Target Word Display */}
            <div 
              className="text-center px-8 py-6 rounded-2xl mb-6 w-full max-w-md"
              style={{ 
            ...
            >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs transition-colors"
                style={{ 
                  backgroundColor: showHint 
                    ? (isDark ? 'rgba(251, 191, 36, 0.15)' : 'rgba(251, 191, 36, 0.1)')
                    : (isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'),
                  color: showHint ? 'rgb(251, 191, 36)' : (isDark ? 'rgb(156, 163, 175)' : 'rgb(107, 114, 128)')
                }}
              >
                <Lightbulb size={14} />
            ...
            <div className="flex gap-3">
              ...
              {isCorrect !== null && currentWordIndex < totalWordsInLevel - 1 && (
                <button
